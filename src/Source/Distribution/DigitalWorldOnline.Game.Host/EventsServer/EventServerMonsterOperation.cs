...
namespace DigitalWorldOnline.GameHost.EventsServer
{
    public sealed partial class EventServer
    {
                ...
                case MobActionEnum.Wait:
                    {
                        ...
                        {
                            map.AttackNearbyTamer(mob, mob.TamersViewing, _assets.NpcColiseum); //attack tamer
                        }
                    }
                    break;
                    ...
                    if (itemDrop.Chance >= UtilitiesFunctions.RandomDouble())
                    {
                        if (targetClient.Tamer.HasAura && targetClient.Tamer.Aura.ItemInfo.Section == 2100)
                        {
                            ...
                            if (targetClient.Tamer.Inventory.AddItem(newItem))
                            {
								...
								_sender.Send(new UpdateItemsCommand(itemClone));
                                _logger.Verbose(
        ...
        private async Task BlessingDATS(MobConfigModel mob, string attackerName, int attackerType)
        {
            if (mob.Bless2)
            {
                var mapIds = new List<int> { 3, 2 }; // Lista fixa dos mapas desejados
