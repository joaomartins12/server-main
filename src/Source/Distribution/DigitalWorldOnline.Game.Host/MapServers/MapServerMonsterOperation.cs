    ...
    public sealed partial class MapServer
    {
                ...
                case MobActionEnum.Respawn:
                    {
                        ...
                        // Verificando se o mob tem o tipo correto para o anúncio
                        if (mob.Type == 62170)
                        {
                            // Se o mob tiver o tipo 62170, ele será anunciado sem verificar o tempo de respawn
                            _logger.Information($"Mob {mob.Name} renasceu. Enviando aviso global.");

                            // Envia uma mensagem global para todos os clientes conectados
                            foreach (var currentMap in Maps)
                            {
                                // Verifica se o mapa tem clientes conectados
                                if (currentMap.Clients.Any())
                                {
                                    var clients = currentMap.Clients;

                                    // Envia a mensagem global para todos os clientes no mapa
                                    foreach (var client in clients)
                                    {
                                        // Verifique se o cliente está válido
                                        if (client != null)
                                        {
                                            // Envia a mensagem global para o cliente
                                            client.Send(new GlobalMessagePacket(mob.Type, 3));
                                        }
                                    }
                                }
                            }

                            // Envia o aviso para o Discord (ajuste conforme sua necessidade)
                            // Envia o aviso para o Discord (ajuste conforme sua necessidade)
                            CallDiscordWarnings("Digital Universe - Raids", $"[SPAWN] {mob.Name} nasceu em {map.Name} CH{map.Channel}.", "81ffcf", "1373314902977937454", "1374537846098100224", mob.Model);
                        }
                        else if (mob.RespawnInterval > 2399)
                        {
                            // Caso o mob não seja do tipo 62170, verifica o tempo de respawn (se for maior que 2399)
                            _logger.Information($"Mob {mob.Name} renasceu. Enviando aviso global.");
                        ...
                        /*else
                        {
                            _logger.Information($"O mob {mob.Name} renasceu, mas o tempo de respawn ({mob.RespawnInterval}) não atingiu o limite para aviso.");
                        }*/
                    }
                    break;
                ...
                case MobActionEnum.Reward:
                    {
                        ...
                        if (mob.RespawnInterval > 3599)
                        {
                            ...
                            CallDiscordWarnings("Digital Universe - Raids", $"[DEAD] {mob.Name} foi morto em {map.Name} CH{map.Channel}. \nNascerá novamente as {formattedTime} (UTC-03), em {(int)(mob.RespawnInterval / 3600)}hrs!", "ff0000", "1374537846098100224", "1374537846098100224", mob.Model);
                        }
        ...
        private void BitDropReward(GameMap map, MobConfigModel mob, GameClient? targetClient)
        {
            ...
            double vipMultiplier = targetClient.AccessLevel switch
            {
                ...
                Commons.Enums.Account.AccountAccessLevelEnum.Vip5 => 1.5,
                Commons.Enums.Account.AccountAccessLevelEnum.Administrator => 1.0,
                _ => 1.0
            };

            if (bitsReward != null && bitsReward.Chance >= UtilitiesFunctions.RandomDouble())
            {
                var amount = (int)(UtilitiesFunctions.RandomInt(bitsReward.MinAmount, bitsReward.MaxAmount) * vipMultiplier);
        ...
        private void ItemDropReward(GameMap map, MobConfigModel mob, GameClient? targetClient)
        {
            ...
            {
                if (now >= globalDrop.StartTime && now <= globalDrop.EndTime &&
            ...
            int vipMultiplier = targetClient.AccessLevel switch
            {
                ...
                Commons.Enums.Account.AccountAccessLevelEnum.Vip2 => 2,
                Commons.Enums.Account.AccountAccessLevelEnum.Vip3 => 3,
                Commons.Enums.Account.AccountAccessLevelEnum.Vip4 => 4,
                Commons.Enums.Account.AccountAccessLevelEnum.Vip5 => 5,
            ...
            while (dropped < totalDrops)
            {
                    ...
                    if (itemDrop.Chance >= UtilitiesFunctions.RandomDouble())
                    {
                        if (targetClient.Tamer?.HasAura == true && targetClient.Tamer.Aura?.ItemInfo?.Section == 2100)
                        {
                            ...
                            if (existingItem != null)
                            {
                                ...
                                _sender.Send(new UpdateItemCommand(existingItem));
                            }
                            else
                            {
                                ...
                                if (emptySlot != -1)
                                {
                                    ...
                                    _sender.Send(new UpdateItemCommand(newItem));
                                }
                                else
                                {
                ...
                if (!droppedThisCycle)
                {
                    //_logger.Warning($"No items could be dropped from mob {mob.Id}. Breaking out of drop loop.");

                    // Verifica se há itens com chance 0% ou menor
                    var zeroChanceItems = itemsReward.Where(x => x.Chance <= 0).ToList();
                    if (zeroChanceItems.Any())
                    {
                        var mapConfig = _sender.Send(new GameMapConfigByMapIdQuery(mob.CurrentLocation.MapId));
                        ...
                        //     message,
                        //       "FF0000",
                        //       "1383566065350742106", // ID do canal
                        //      "1383566065350742106", // ID de fallback ou log
                        //     mob.Model // model do mob
                        //   );
                    }
                    break;
                }
                    ...
                    if (itemDrop.Chance >= UtilitiesFunctions.RandomDouble())
                    {
                        foreach (var questInProgress in tamer.Progress.InProgressQuestData)
                        {
                            ...
                            if (goalIndex != -1)
                            {
                                ...
                                // Remove este item da lista para não cair de novo para o mesmo tamer
                                itemsReward.RemoveAll(x => x.Id == itemDrop.Id);
                                break;
                            }
                        }
            ...
            if (mob.Bless2)
            {
                ...
                foreach (var mapId in mapIds)
                {
                    ...
                    foreach (var currentMap in mapChannels)
                    {
                        ...
                        var targetItem = _assets.ItemInfo.FirstOrDefault(itemAssetModel => itemAssetModel.ItemId == 50063);
        ...
        private void BitDropReward(GameMap map, SummonMobModel mob, GameClient? targetClient)
        {
            ...
            int vipMultiplier = targetClient.AccessLevel switch
            {
                Commons.Enums.Account.AccountAccessLevelEnum.Vip => 2,
