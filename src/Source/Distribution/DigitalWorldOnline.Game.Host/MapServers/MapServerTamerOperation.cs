    ...
    public sealed partial class MapServer
    {
        ...
        private void ReedemTimeReward(GameClient client)
        {
            ...
            foreach (var drop in drops)
            {
                ...
                {
                    _logger.Warning($"Item info not found for ID {drop.ItemId} during time reward.");
                    continue;
                }
                ...
                var existingItem = inventory.Items.FirstOrDefault(x =>
                    x.ItemId == reward.ItemId &&
                    x.ItemInfo?.Overlap > 1 &&
                    x.Amount < x.ItemInfo.Overlap);

                if (existingItem != null)
                {
                    existingItem.IncreaseAmount(reward.Amount);

                    var tempItem = (ItemModel)reward.Clone();
                    ...
                    _ = _sender.Send(new UpdateItemCommand(existingItem));
                }
                else
                {
                    ...
                    if (emptySlot != -1)
                    {
                        reward.SetSlot(emptySlot);
                        inventory.InsertItem(reward);

                        client.Send(new ReceiveItemPacket(reward, InventoryTypeEnum.Inventory, emptySlot));
                        _ = _sender.Send(new UpdateItemCommand(reward));
                    }
                    else
                    {
                        client.Send(new PickItemFailPacket(PickItemFailReasonEnum.InventoryFull));
                        client.Send(new SystemMessagePacket("Seu inventário está cheio."));
                        return;
                    }
                }
            }
