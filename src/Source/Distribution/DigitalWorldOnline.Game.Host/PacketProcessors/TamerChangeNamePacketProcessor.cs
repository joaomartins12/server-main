    ...
    public class TamerChangeNamePacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            if (inventoryItem != null)
            {
                ...
                // Enviar log para o Discord sobre a mudança de nome
                await _mapServer.CallDiscord(
                    $"**Nome Alterado com sucesso para**\n" +
                    $"O tamer **{oldName}** alterou seu nome para **{newName}**.\n" +
                    $"**ID do Tamer**: {client.Tamer.Id}\n" +
                    $"**Jogador**: {client.Tamer.Name} ({client.AccountId})",
                    client,
                    "00ff00",  // Cor de sucesso (verde)
                    "TAMER NAME CHANGE",  // Título do log
                    "1374552601970085999" // ID do canal Discord (substitua pelo canal correto)
                );
