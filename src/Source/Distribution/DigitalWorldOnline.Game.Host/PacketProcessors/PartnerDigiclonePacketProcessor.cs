...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class PartnerDigiclonePacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            // Backup só é consumido se realmente foi usado
            if (cloneResult == DigicloneResultEnum.Backup && backupItem != null)
            {
                client.Tamer.Inventory.RemoveOrReduceItem(backupItem, 1, backupSlot);
            }

            // ---- FIX: salvar sempre clone + itens + bits ----
            await _sender.Send(new UpdateDigicloneCommand(client.Partner.Digiclone));
            await _sender.Send(new UpdateItemCommand(digicloneItem));
            if (backupItem != null)
            {
                await _sender.Send(new UpdateItemCommand(backupItem));
            }
