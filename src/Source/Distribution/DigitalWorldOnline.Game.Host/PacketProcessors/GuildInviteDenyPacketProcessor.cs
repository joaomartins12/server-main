    ...
    public class GuildInviteDenyPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            var targetClient = _mapServer.FindClientByTamerName(senderName) ??
                _dungeonsServer.FindClientByTamerName(senderName) ??
                _pvpServer.FindClientByTamerName(senderName) ??
                _eventServer.FindClientByTamerName(senderName);

            if (targetClient == null)
            {
                _logger.Debug($"targetClient not found");
                return;
            }
