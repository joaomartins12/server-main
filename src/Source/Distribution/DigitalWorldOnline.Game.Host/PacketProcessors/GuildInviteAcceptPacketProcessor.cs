...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class GuildInviteAcceptPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            _logger.Debug($"GuildID = {guildId} | inviterName: {inviterName}");

            var inviterCharacter = _mapServer.FindClientByTamerName(inviterName) ??
                _dungeonsServer.FindClientByTamerName(inviterName) ??
                _pvpServer.FindClientByTamerName(inviterName) ??
                _eventServer.FindClientByTamerName(inviterName);
