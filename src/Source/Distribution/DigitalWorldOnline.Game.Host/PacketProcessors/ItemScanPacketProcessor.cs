...
namespace DigitalWorldOnline.Game.PacketProcessors
{
    public class ItemScanPacketProcessor : IGamePacketProcessor
    {
        ...
        public async Task Process(GameClient client, byte[] packetData)
        {
            ...
            {
                _logger.Error($"Invalid item on slot {slotToScan} for tamer {client.TamerId} - {client.Tamer.Name} on scanning.");
            ...
            {
                await _mapServer.CallDiscord($"try DUPPING in SCANN {amountToScan}x {scannedItem.ItemInfo.Name}, but he has {scannedItem.Amount}x!", client, "e06666", "DISCONNECTED", "1374551861683683338");

                _logger.Error($"Inv. itens count: {client.Tamer.Inventory.CountItensById(scannedItem.ItemId)} < amountToScan: {amountToScan}.");
            ...
            var dropList = string.Join(',', receivedRewards.Select(x => $"{x.Value.ItemInfo.Name} x{x.Value.Amount}"));
            ...
            // ✅ Log no Discord da ação de scan          

            var scanLog = $"**Item Escaneado**\n" +
                          $"**Jogador**: {client.Tamer.Name}\n" +
                          $"**Item**: {scannedItem.ItemInfo.Name} (ID: {scannedItem.ItemId})\n" +
                          $"**Quantidade Escaneada**: {scannedItens}\n" +
                          $"**NPC**: {npcId}\n" +
                          $"**Custo Total**: {cost} Bits\n" +
                          $"**Recompensas**: {dropList}";

            await _mapServer.CallDiscord(
                scanLog,
                client,
                "00bfff", // Cor azul claro
                "Item Scanner",
                "1374551142318604360" // ← substitua aqui com o ID real do canal
            );
